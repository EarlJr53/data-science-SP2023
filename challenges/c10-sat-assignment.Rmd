...
-   What is the confidence interval on the coefficient of `both_SAT`? Is
    ...
    -   The confidence interval is [0.00198, 0.00349], with an estimate
        of 0.00274. This is extremely close to zero, but given the
        different orders of magnitude of the two values, this is still
        significant.
-   By itself, how well does `both_SAT` predict `univ_GPA`?
    -   `both_SAT` by itself seems to be an okay predictor of
        `univ_GPA`, but not phenomenal.
...
-   How well do these models perform, compared to the one you built in
    q6?
    -   The model with just `high_GPA` seems to perform roughly the same
        as, if not slightly better than the `both_SAT` model, although
        the combined model seems to actually be not significantly
        different from zero.
...
-   Between `both_SAT` and `high_GPA`, which single variable would you
    ...
    -   If I had to choose a single variable, I would likely choose
        ...
        especially the correlation calculations in q4.
-   Is `both_SAT` an effective predictor of `univ_GPA`? What specific
    ...
    -   According to q6 and q7, `both_SAT` appears to actually be an
        ...
        especially the plots, seem to suggest otherwise.
