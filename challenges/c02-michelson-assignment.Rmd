...
-   Write your observations here! - It is relatively straightforward to
    group by Distinctness and summarize the other notable attributes.
    Even though the means aren't exactly the same, due to different
    numbers of significant figures, I can tell that the results are the
    same. - I also notice that Michelson had more higher quality
    readings, but would be curious what the variance within each
    distinctness category is.
-   Why might your table differ from Michelson's?
    -   Michelson's table of values seems to be rounded to the multiple
        of 10 km/s, whereas our data uses significant figures to the
        10th of a km/s.
...
-   Is Michelson's estimate of the error (his uncertainty) greater or
    ...
    -   Michelson's uncertainty is less than the true error, by a factor
        of 3x.
-   Make a quantitative comparison between Michelson's uncertainty and
    his error.
    -   Michelson's uncertainty is almost exactly 1/3 of his actual
        error. He actually overshot the correct value, and the true
        value is not within his uncertainty range. I am not quite sure
        what to attribute this to other than human estimation
        imperfection.
...
-   Similarities
    ...
    -   The mean velocity line also goes outside the uncertainty in both
        plots.
-   Differences
    -   While Michelson's data seems to grow closer and closer aligned
        with time, narrowing toward his calculated solution, the
        simulated plot seems to be much more inconsistent with its
        values over time.
    -   The real data seems to have more clumps of data for specific
        days, indicating potential day-specific factors that may have
        influenced the data on those days, while the random data is less
        clumpy.
...
-   I plotted Velocity (before adding +92km/s) against temperature, with
    a line for the actual speed of light in a vacuum. This actually
    shows that without correcting for the vacuum, the data points taken
    by Michelson are actually closer to the correct value.
-   I also used a violin plot to understand the error in each data point
    as it relates to the distinctness of the data point. The most
    distinct values seem to be more consistent in their error, while the
    less distinct values are more spread in their error, even if the
    error is lowest among the `1` Distinctness values.
