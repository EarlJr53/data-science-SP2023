...
-   Which `class` has the most vehicles?
    -   It seems that SUVs have the most vehicles, but it is hard to
        tell.
-   Which `class` has the broadest distribution of `cty` values?
    -   Subcompacts seem to have the broadest distribution of `cty`
        values.
-   Which graph---`vis1` or `vis2`---best helps you answer each
    question?
    -   `vis1` is more useful in answering the first question, while
        `vis2` is more helpful in answering question 2.
...
mpg %>%
  ggplot(aes(cty, color = class)) +
  geom_density() + facet_wrap(vars(class), scales = "free")
```
...
-   0.005 width: Troughs right below each round number (except 0.6 and
    0.8), with huge peaks at each of the round numbers.

-   0.01 width: See abrupt peaks at 0.3, 0.4, 0.5, 0.7, 0.9, 1.0

-   0.05 width: Highest peak at 0.3-0.35, second highest at 1.0-1.05

-   0.1 width: See much higher peak at between 0.3 and 0.4
